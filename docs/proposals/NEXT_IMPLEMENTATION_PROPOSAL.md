...
15. **感情オブジェクト周辺のパーティクルエフェクト** - 感情タイプに応じたパーティクル、強度に応じた数とサイズ ✅
16. **深度ジャンプ時のトランジションエフェクト** - フェードイン/フェードアウト、スクロール方向に応じたパーティクルエフェクト ✅
...
- 既存の星屑エフェクト（`sparkles.ts`）の実装パターンを参考にできる
...
**技術的な実装詳細**:
- **既存実装の参考**: `src/lib/pixi/sparkles.ts` のパーティクル管理パターンを活用
- **パーティクルデータ構造**:
  ```typescript
  ...
  ```
- **アニメーション方式**: PixiJSの`Graphics`と`Container`を使用し、`app.ticker`でアニメーション
- **パーティクル生成**: 感情オブジェクトの位置を中心に、半径20-40pxの範囲でランダム生成
- **パーティクル更新**: 各フレームで位置、不透明度、回転を更新し、ライフタイムが0になったら再利用または削除

**実装場所**:
- 新規ファイル: `src/lib/pixi/effects/EmotionParticles.ts`
  - `createEmotionParticleSystem()`: パーティクルシステムの作成
  - `updateEmotionParticles()`: パーティクルの更新
  - `generateParticlesForEmotion()`: 感情オブジェクトごとのパーティクル生成
- `src/components/canvas/Scene2DPixi.tsx` に統合
  - 感情オブジェクトの描画後にパーティクルシステムを初期化
  - `app.ticker`でパーティクルを更新
...
**技術的な実装詳細**:
- **深度変化の検出**: `useDepth.ts`で深度の変化量を監視
- **フェードレイヤー**: PixiJSの`Graphics`で画面全体を覆う黒いレイヤーを作成し、`alpha`で制御
- **トランジション管理**: `TransitionState`でトランジションの状態（開始、進行中、完了）を管理
...
6. **既存実装の活用**: 星屑エフェクト（`sparkles.ts`）の実装パターンを参考にする
...
- [x] 感情オブジェクト周辺のパーティクルエフェクト ✅
  - [x] 感情タイプに応じたパーティクル
  - [x] 感情の強度に応じたパーティクルの数とサイズ
  - [x] パーティクルのライフタイム管理
  - [x] 動作確認
- [x] 深度ジャンプ時のトランジションエフェクト ✅
  - [x] フェードイン/フェードアウト
  - [x] スクロール方向に応じたパーティクルエフェクト
  - [x] 深度マップからのジャンプ時の視覚的フィードバック
  - [x] 動作確認
...
- **実装難易度**: 高
- **実装時間**: 1-2週間
- **効果**: フレームレートの向上、メモリ使用量の削減、スムーズなスクロール体験
...
**推奨**: パフォーマンス最適化を優先することを推奨します。大量のオブジェクトを表示する際のパフォーマンス向上により、ユーザー体験が大幅に改善されます。
